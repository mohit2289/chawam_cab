...
(function($) {
	...
	if($(window).width() <= 991) {
		...
		function init() {
			...
			$('.main-nav a').on('click', function(e) {
				if($(this).parent().hasclassName('has-submenu')) {
					e.preventDefault();
				}
				if(!$(this).hasclassName('submenu')) {
					$('ul', $(this).parents('ul:first')).slideUp(350);
					$('a', $(this).parents('ul:first')).removeclassName('submenu');
					$(this).next('ul').slideDown(350);
					$(this).addclassName('submenu');
				} else if($(this).hasclassName('submenu')) {
					$(this).removeclassName('submenu');
	...
	$(".btn.filterbtn").on('click', function() {
		  $(".showfilter").toggleclassName("filter-open");
	});		
	
	// Password Eye

	$('.toggle-password').on('click', function() {
		$(this).toggleclassName("feather-eye feather-eye-off");
	...
	if($('.about-testimonials').length > 0) {
		$('.about-testimonials').owlCarousel({
			...
			autoplay:true,
			smartSpeed: 2000,
			navText : ["<i className='fa-solid fa-angle-left'></i>","<i className='fa-solid fa-angle-right'></i>"],
			responsive:{
				0:{
					items:1
				},
